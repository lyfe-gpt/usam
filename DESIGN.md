...
- **Calculator kit** (`components/calculators/fields.jsx`) — shared `NumberField`, `ResultPanel`, `InputCard`, `Row`, `fmt`/`pct`, `monthlyPI`, and `num` (NaN→0 coalescer). All six calculators (DSCR, fix/flip, BRRRR, hard-money-cost, cap-rate, cash-out-refi) compose from this; reuse it for any new one rather than re-styling. `NumberField` allows a blank field (stored as NaN, shown empty so clearing lets you type fresh), clamps negatives to 0, takes an optional `max` for percentage clamps, and caps every field at **$10B (1e10)** by default so sums/products can't overflow to Infinity and labels never render in scientific notation; every calculator wraps its inputs in `num()` so a blank field never renders NaN, and raw `.toFixed()` ratio displays (DSCR ratio, fix/flip ROI) are coalesced to finite. **Mobile-first layout:** `.calc-grid` is inputs-left / result-right on desktop; at ≤860px it stacks and the **result panel moves first (`order:-1`)** so the headline figure leads in the mobile hero, inputs follow. `CalculatorDetail` keeps the hero compact (eyebrow + h1 + tagline) and moves the explainer paragraph below the tool.
