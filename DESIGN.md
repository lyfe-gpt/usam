...
- **Calculators** (`components/calculators/DscrCalculator.jsx`, `FixFlipCalculator.jsx`) — interactive input panel (white card, 2-col `.calc-inputs` → 1-col at ≤640px) beside a dark sticky result panel (`#0E1A2B→#15294A` gradient, big Schibsted figure, pass/fail pill, line-item breakdown, apply CTA). Wrapper `.calc-grid` (1.1fr / 0.9fr) stacks at ≤860px and drops the result panel's `position: sticky`. Input adornments use `#667085` (NOT `#8A97A8`) so the `$`/`%` glyphs clear WCAG AA on white. Hub + detail pages in `pages/Calculators.jsx` / `pages/CalculatorDetail.jsx`; each detail page emits breadcrumb + `FAQPage` JSON-LD. **Mobile (≤640px) uses a step wizard + sticky result** — see Calculator kit.
...
- **Calculator kit** (`components/calculators/fields.jsx`) — shared `NumberField`, `ResultPanel`, `InputCard`, `Row`, `fmt`/`pct`, `monthlyPI`, `num` (NaN→0 coalescer), `applyUrl`, and the mobile-only `useIsMobile` / `StickyResult` / `Wizard`. All six calculators (DSCR, fix/flip, BRRRR, hard-money-cost, cap-rate, cash-out-refi) compose from this; reuse it for any new one rather than re-styling. `NumberField` allows a blank field (stored as NaN, shown empty so clearing lets you type fresh), clamps negatives to 0, takes an optional `max` for percentage clamps, and caps every field at **$10B (1e10)** by default so sums/products can't overflow to Infinity and labels never render in scientific notation; every calculator wraps its inputs in `num()` so a blank field never renders NaN, and raw `.toFixed()` ratio displays (DSCR ratio, fix/flip ROI) are coalesced to finite. **Responsive layout:** `.calc-grid` is inputs-left / result-right on desktop. **Tablet (641-860px):** stacks and the **result panel moves first (`order:-1`)** so the headline figure leads, inputs follow. **Phone (≤640px): a distinct mobile-only treatment** driven by `useIsMobile(640)` (a live `matchMedia` hook) — desktop/tablet are untouched, all gating is `matchMedia` + the scoped `@media` queries:
  - **`StickyResult`** — a compact dark bar pinned just under the header (`position: sticky; top: 56`) showing the headline number + verdict pill + apply CTA, so the live answer stays in view while the visitor edits below. Each calculator renders it only when `mobile` is true; the full result panel still flows below the inputs for the detailed breakdown.
  - **`Wizard`** — a multi-step input stepper (2-3 fields per step, Back/Next, tappable progress dots, "All set" on the last step). Each calculator passes `steps={[{ title, content }]}`. On desktop `Wizard` is a **no-op**: it returns every field inline so the inputs grid renders exactly as before. Result updates live as you step (the sticky bar reflects every keystroke). Step groupings per calculator are defined in each component.
  - `CalculatorDetail` keeps the hero compact (eyebrow + h1 + tagline) and moves the explainer paragraph below the tool.
- **CalculatorCta** (`components/CalculatorCta.jsx`) — blue CTA band placed above the FAQ on a program page, linking to that program's calculator(s) (matched via each calculator's `relatedProgram`). Renders nothing for programs with no related calculator. Primary button + "More tools" pills for additional calculators.
- **Calculator → Apply prefill** — each calculator's apply CTA uses `applyUrl(program, fields)` (in `fields.jsx`) to build `/apply?program=…&purchase=…&rehab=…&arv=…&monthlyRent=…&loanAmount=…` from its current inputs. `Apply.jsx`'s mount effect reads those params (keys match the numbers-step field keys) and prefills the form, then skips the loan-type step since the program is known. So a visitor who runs the numbers lands on the apply flow with their figures already filled in.
